...
def test_configured_provider_resolves_to_the_builtin_pgvector_backend() -> None:
    config = build_config(config_path="config/mindmemos_lite/dev.example.yaml")
    registry = BackendRegistry()
    register_builtin_backends(registry)

    backend = registry.create(
        config=build_backend_config(config.database),
        tables=_tables(),
    )

    assert registry.providers == ("pgvector",)
    assert backend.name == "pgvector"
    assert backend.capabilities.max_vector_dimensions == 16_000
...
def test_pgvector_preserves_service_graph_node_identity_fields() -> None:
    tables = build_graph_registry()
    backend = PgVectorBackend(
        options=PgVectorOptions(dsn="postgresql://unused"),
        tables=tables,
        pool=object(),
    )
    spec = tables.get("graph_node")
    record = Record(
        table="graph_node",
        record_id='node:["Memory","memory-1"]',
        scope=DatabaseScope(project_id="project-a"),
        payload={"node_id": "memory-1", "node_type": "Memory"},
    )

    values = backend._prepare_record(spec, record)

    assert values[2:] == ('node:["Memory","memory-1"]', "memory-1", "Memory")
    indexes = [statement.as_string() for statement in backend._index_ddl(spec)]
    assert any('(_scope_key, "node_type", "node_id")' in statement for statement in indexes)
