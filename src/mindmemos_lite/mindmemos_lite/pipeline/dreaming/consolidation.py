"""Memory consolidation pipeline for resolving related memory issues."""
...
from ...components.memory_modeling.vanilla.edges import property_relationships
...
class EntityMemoryCluster:
    """One active entity-centered cluster assembled by Dreaming."""
...
@register(type="dreaming", name="memory_consolidation")
class MemoryConsolidationPipeline(MemoryPersistencePipelineMixin):
    ...
    def __init__(
        ...
        *,
        ...
        self._operation_recorder = operation_recorder or MemoryOperationRecorder.from_service(persistence.service)
        self._consistency = consistency or _default_consistency()

    @classmethod
    def from_config(cls, config, **kwargs):
        return cls(
            dreaming_config=config.algo_config.dreaming,
    ...
    async def dream(self, inp: DreamingPipelineInput, context: MemoryRequestContext) -> DreamingPipelineResult:
        if inp.mode == "async":
            raise RuntimeError("async dreaming must be dispatched through the Lite memory service")
        return await self.dream_sync(inp, context)
    ...
    async def _cluster_hot_memories(
        ...
        2. One persistence query: get all ``Memory -> Entity`` edges for hot
           seeds and their 1-hop entity neighbors.
        ...
        # Step 2: for each hot seed, find its entities and all neighbor
        # memories that share those entities (1-hop).
        rows = await self._list_entity_memory_clusters(
            context,
            list(seed_memory_ids),
            limit_per_entity=self._cfg.max_entity_memory_count + 1,
        )
        ...
        for row in rows:
            ...
            mem_ids = [str(m) for m in row.memory_ids if m]
        ...
        # Batch read all memories through Lite persistence in one call
        memory_view_map: dict[str, MemoryView] = {}
        if all_memory_ids:
            for mv in await self.persistence.get_memories(context, list(all_memory_ids)):
                if mv.status == "active":
    ...
    async def _list_memories_by_shared_entities(
        self,
        context: MemoryRequestContext,
        memory_ids: list[str],
        *,
        ...
        """Build seed/entity groupings from the generic relation query."""

        seed_ids = list(dict.fromkeys(memory_id for memory_id in memory_ids if memory_id))
        if not seed_ids or limit_per_entity <= 0:
            return []
        rows = await self.persistence.get_related_memory_ids(
            context,
            seed_ids,
            steps=(
            ...
            max_candidates=max(1, len(seed_ids) * limit_per_entity * 8),
        )
        grouped: dict[tuple[str, str], list[str]] = defaultdict(list)
        metadata: dict[tuple[str, str], tuple[str | None, str | None]] = {}
        for row in rows:
            seed_id = str(row.get("seed_memory_id") or "")
            entity_id = str(row.get("entity_id") or "")
            memory_id = str(row.get("memory_id") or "")
            if not seed_id or not entity_id or not memory_id or (not include_seed and memory_id == seed_id):
                continue
            key = (seed_id, entity_id)
            metadata.setdefault(
                key,
                (
                    str(row["entity_name"]) if row.get("entity_name") is not None else None,
                    str(row["entity_type"]) if row.get("entity_type") is not None else None,
                ),
            )
            values = grouped[key]
            if memory_id not in values and len(values) < limit_per_entity:
                values.append(memory_id)
        return [
            GraphNeighborScope(
                seed_memory_id=seed_id,
                entity_id=entity_id,
                entity_name=metadata[(seed_id, entity_id)][0],
                entity_type=metadata[(seed_id, entity_id)][1],
    ...
    @traced("dreaming.list_entity_clusters")
    async def _list_entity_memory_clusters(
        self,
        context: MemoryRequestContext,
        ...
        *,
        ...
        """Collapse per-seed shared-entity paths into Dreaming-owned clusters."""

        scopes = await self._list_memories_by_shared_entities(
            context,
            seed_memory_ids,
            include_seed=True,
            active_only=True,
        ...
        for scope in scopes:
            ...
            related_ids = grouped[scope.entity_id]
            for memory_id in (scope.seed_memory_id, *scope.memory_ids):
                if memory_id not in related_ids and len(related_ids) < limit_per_entity:
    ...
    async def _pending_consolidation_add_record_ids(
        ...
        records = await self._operation_recorder.add_records.get_many(context, unique_ids)
        done_ids = {
            record.add_record_id
            for record in records
            if record.consolidation_status.lower() == "done"
        }
        found_ids = {record.add_record_id for record in records}
    ...
    def _get_activity_collector(self) -> RecentActivityCollector:
        if self._activity_collector is None:
            self._activity_collector = RecentActivityCollector(self._operation_recorder)
    ...
    async def _mark_scope_add_records_done(self, context, scope, run_id, marked_add_record_ids) -> int:
        ...
        for aid in scope.add_record_ids:
            if aid in marked_add_record_ids:
                continue
            await self._operation_recorder.patch_add_record(
                context,
                aid,
                {
