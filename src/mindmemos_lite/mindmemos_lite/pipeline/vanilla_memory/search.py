...
from ...config import MemoryConfig, TextProcessingConfig, VanillaSearchConfig, get_config
from ...config.vanilla import (
...
class SearchEngineOptions:
    """Optional overrides internal to vanilla search orchestration."""
...
class VanillaSearchEngine(MemoryPersistencePipelineMixin):
    ...
    def from_config(cls, config: MemoryConfig, **kwargs):
        return cls(
            ...
            search_config=config.algo_config.search,
            **kwargs,
        )
    ...
    def _get_vanilla_search_config(self) -> VanillaSearchConfig:
        ...
        config = get_config().algo_config.search
    ...
    async def search_candidates(
        ...
        *,
        ...
        if dense_vector is not None:
            ...
            result = await self.persistence.search_hybrid(
                context,
                query,
    ...
    async def _with_graph_related_hits(
        ...
        try:
            related_memories = await self.persistence.get_memories(context, candidate_ids)
        except Exception:
    ...
    async def _lineage_for_existing_hits(
        ...
        try:
            lineage_by_id = await self.persistence.get_memory_lineage(context, seed_ids)
            archived_ids = _ordered_archived_ids(lineage_by_id, existing_ids=set(seed_ids))
            archived_memories = await self.persistence.get_memories(context, archived_ids) if archived_ids else []
        except Exception:
...
@register(type="search", name="vanilla_search")
class VanillaSearchPipeline(MemoryPersistencePipelineMixin):
    """Thin top-level adapter around the copied vanilla search engine."""

    def __init__(
        self,
        *,
        engine: VanillaSearchEngine | None = None,
        ...
        **kwargs,
        ...
        super().__init__(**kwargs)
        self._engine = engine or VanillaSearchEngine(
            text_config=text_config,
            search_config=search_config,
            embed_client=embed_client,
            persistence=self.persistence,
        )
        self._final_filter = final_filter or SearchFinalFilter(
    ...
    async def search(
        ...
        candidates = await self._engine.search_candidates(inp, context)
        memories = await self._final_filter.apply(
            query=inp.query,
            candidates=candidates,
            top_k=inp.top_k,
            rerank=inp.rerank and self._engine._get_vanilla_search_config().use_reranker,
            score_threshold=inp.score_threshold,
        )
        return SearchPipelineResult(status="ok", memories=memories)


def _optional_rerank_client():
    try:
