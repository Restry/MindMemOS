...
from ..logging import get_logger, get_tracer, traced
...
class RerankClient:
    ...
    async def _rerank_single_batch(
        ...
        **kwargs: Any,
        ...
        start = perf_counter()
        with tracer.start_as_current_span("llm.rerank.provider", record_exception=False) as provider_span:
            provider_span.set_attribute("llm.call", True)
            provider_span.set_attribute("llm.operation", "rerank")
            provider_span.set_attribute("llm.model", self._default_model)
            provider_span.set_attribute("llm.rerank.document_count", len(documents))
            try:
                ...
                provider_span.set_status(Status(StatusCode.ERROR, str(exc)))
                logger.info(
                    "litellm_call",
                    kind="rerank",
                    model=self._default_model,
            ...
            provider_span.set_attribute("llm.usage.prompt_tokens", int(usage.prompt_tokens or 0))
            provider_span.set_attribute("llm.usage.completion_tokens", int(usage.completion_tokens or 0))
            provider_span.set_attribute("llm.usage.total_tokens", int(usage.total_tokens or 0))
