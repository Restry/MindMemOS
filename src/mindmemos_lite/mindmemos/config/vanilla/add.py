...
@dataclass
class VanillaAddChunkerConfig:
    """Configuration owned by the vanilla add chunker components.
    ...
    Used in:
        - ``MemoryConfig.algo_config.add.chunker`` for YAML-driven configuration
        - ``TurnGrouper``, ``ChunkPlanner``, ``HistoryPacker``,
          ``LongTurnCompactor``, and ``LongTurnSummarizer``
    """
...
class VanillaAddConfig:
    ...
    chunker: VanillaAddChunkerConfig = field(default_factory=VanillaAddChunkerConfig)
