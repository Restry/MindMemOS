...
from ..base import SearchPipeline
from .executor import FeedbackActionExecutor


class _PendingFeedbackActivityStore:
    """Filter processed add records at Lite's backend-neutral recorder boundary."""

    def __init__(self, recorder: MemoryOperationRecorder) -> None:
    ...
    async def list_activity_records(
        ...
        *,
        ...
        records = await self._recorder.list_activity_records(
            kind,
            scope,
            ...
            feedback_processed=False if kind == "add" else None,
        )
        if kind == "add":
            pending = [record for record in records if not bool(record.payload.get("feedback_processed", False))]
            return pending if max_records is None else pending[:max_records]
        return records
...
class ImplicitFeedbackRecordCollector:
    ...
    async def mark_feedback_processed(self, context: MemoryRequestContext, add_record_ids: list[str]) -> None:
        ...
        for add_record_id in add_record_ids:
            await self._recorder.patch_add_record(
                context,
                add_record_id,
    ...
    def _recorder(self) -> MemoryOperationRecorder:
        if self._operation_recorder is None:
            raise RuntimeError("implicit feedback collection requires the Lite operation recorder")
        return self._operation_recorder

    @property
    def _collector(self) -> RecentActivityCollector:
        if self._activity_collector is None:
            self._activity_collector = RecentActivityCollector(
                _PendingFeedbackActivityStore(self._recorder)
            )
    ...
    def _search_pipeline(self) -> SearchPipeline:
        if self._search is None:
            raise RuntimeError("implicit feedback requires a search pipeline supplied by the Lite service")
        return self._search
