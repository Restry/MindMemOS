...
from ..logging import add_span_event, get_logger, get_tracer, traced
    ...
    async def chat(
        ...
        **kwargs: Any,
        ...
        while True:
            ...
            with tracer.start_as_current_span("llm.chat.provider", record_exception=False) as provider_span:
                provider_span.set_attribute("llm.call", True)
                provider_span.set_attribute("llm.operation", "chat")
                provider_span.set_attribute("llm.task", task)
                provider_span.set_attribute("llm.model", target)
                provider_span.set_attribute("llm.attempt", attempt)
                try:
                ...
                _annotate_llm_usage(provider_span, task=task, model=model_name, usage=usage)
...
def _annotate_llm_usage_span(*, task: str, model: str, usage: Usage) -> None:
    ...
    _annotate_llm_usage(span, task=task, model=model, usage=usage)


def _annotate_llm_usage(span: trace.Span, *, task: str, model: str, usage: Usage) -> None:
    """Attach normalized LLM dimensions to one span."""
