...
def _resolved_expected_dim() -> int | None:
    """Resolve the embedding dimension declared by the active model router.

    Returns the shared explicit endpoint dimension, or ``None`` when config is
    uninitialized or endpoints leave the provider-native dimension unspecified.
    """

    try:
        dimensions = {
            endpoint.dimensions
            for endpoint in get_config().embed_model_router.endpoints
    ...
    async def embed(
        ...
        **kwargs: Any,
        ...
        attrs = {
            ...
            "llm.model": target,
            **_input_stats(text),
            **_router_attrs(self._router, target),
        }
        add_span_event(
            "llm.embed.input",
            {
        ...
        def annotate_provider_span(span: trace.Span, response: Any) -> None:
            response_usage = usage_tokens(getattr(response, "usage", None))
            response_model = get_response_value(response, "model", target) or target
            span.set_attribute("llm.model", response_model)
            span.set_attribute("llm.usage.prompt_tokens", int(response_usage.prompt_tokens or 0))
            span.set_attribute("llm.usage.completion_tokens", int(response_usage.completion_tokens or 0))
            span.set_attribute("llm.usage.total_tokens", int(response_usage.total_tokens or 0))
        ...
        try:
            resp = await traced_awaitable(
                ...
                on_success=annotate_provider_span,
            )
