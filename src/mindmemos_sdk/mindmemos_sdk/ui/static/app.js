const TAB_COPY = {
  ...
  lite: {
    title: "Lite Logs",
    description: "Inspect logs and traces generated by the local Lite runtime.",
  },
  settings: {
    title: "Settings",
...
let liteSpanPage = 1;
let liteExpandedSpanIds = new Set();
...
const liteTraceKey = (span) => `${span.source}:${span.trace_id}:${span.span_id}`;
...
const renderLiteTraceList = (payload) => {
  ...
  const filterSuffix = payload.span_name ? ` matching “${payload.span_name}”` : "";
  document.querySelector("#lite-trace-count").textContent = `${payload.total || 0} span${payload.total === 1 ? "" : "s"}${filterSuffix}`;
  document.querySelector("#lite-page-status").textContent = `Page ${payload.page || 1} of ${payload.total_pages || 1}`;
  document.querySelector("#lite-previous-page").disabled = !payload.has_previous;
  document.querySelector("#lite-next-page").disabled = !payload.has_next;
  if (!spans.length) {
    list.innerHTML = '<div class="lite-empty-state"><div class="empty-icon small">⌁</div><strong>No spans found</strong><span>Try a different span name or trace directory.</span></div>';
    return;
  }

  list.innerHTML = spans.map((span) => {
    const key = liteTraceKey(span);
    const active = key === activeLiteTraceKey ? " active" : "";
    const rootRelation = span.root_span_status === "pending"
      ? "Root pending · trace in progress"
      : span.root_span_status === "missing"
        ? "Root unavailable · partial trace"
        : span.is_root
          ? "This span is the root"
          : `Root: ${span.root_span_name}`;
    const status = span.status_code === "ERROR" ? " · ERROR" : "";
    return `<button class="lite-trace-row${active}" type="button" data-trace-key="${escapeHtml(key)}" data-trace-id="${escapeHtml(span.trace_id)}" data-span-id="${escapeHtml(span.span_id)}" data-trace-source="${escapeHtml(span.source)}">
      <span class="lite-trace-name" title="${escapeHtml(span.name)}">${escapeHtml(span.name)}</span>
      <span class="lite-trace-duration">${escapeHtml(formatDuration(span.duration_ms))}</span>
      <span class="lite-trace-id">traceId ${escapeHtml(span.trace_id)}</span>
      <span class="lite-trace-generated-time">Generated <time datetime="${escapeHtml(span.start_time)}" title="${escapeHtml(span.start_time)}">${escapeHtml(formatTraceTime(span.start_time))}</time></span>
      <span class="lite-trace-meta">${escapeHtml(rootRelation)}${escapeHtml(status)}</span>
    </button>`;
  }).join("");

  list.querySelectorAll("[data-span-id]").forEach((row) => {
    row.addEventListener("click", () => loadLiteTraceDetail({
      trace_id: row.dataset.traceId,
      span_id: row.dataset.spanId,
      source: row.dataset.traceSource,
    }));
  });
};

const loadLiteTraces = async ({ page = 1 } = {}) => {
  const directory = document.querySelector("#lite-trace-directory").value.trim();
  const limitInput = document.querySelector("#lite-trace-limit");
  const limit = Math.min(500, Math.max(1, Number(limitInput.value) || 25));
  const spanName = document.querySelector("#lite-span-name-filter").value.trim();
  const list = document.querySelector("#lite-trace-list");
  if (!directory) {
  ...
  liteSpanPage = Math.max(1, Number(page) || 1);
  const requestToken = ++liteTraceRequestToken;
  activeLiteTraceKey = null;
  activeLiteSpanId = null;
  liteTraceDetailState = null;
  liteExpandedSpanIds = new Set();
  document.querySelector("#lite-database-badge").textContent = "LOADING";
  document.querySelector("#lite-root-context").classList.add("hidden");
  list.innerHTML = '<div class="lite-empty-state"><div class="empty-icon small">⌁</div><strong>Scanning Lite spans…</strong><span>Opening matching SQLite databases in read-only mode.</span></div>';
  ...
  document.querySelector("#lite-span-detail").innerHTML = '<div class="lite-empty-state compact"><strong>Span details</strong><span>Select a span, then click any flame block.</span></div>';
  setLiteNotice("Scanning the selected directory for MindMemOS Lite trace databases…");
  try {
    const params = new URLSearchParams({
      directory,
      limit: String(limit),
      offset: String((liteSpanPage - 1) * limit),
    });
    if (spanName) params.set("span_name", spanName);
    const payload = await apiRequest(`/api/v1/lite/spans?${params.toString()}`);
    if (requestToken !== liteTraceRequestToken) return;
    liteSpanPage = payload.page || liteSpanPage;
    renderLiteTraceList(payload);
    setLiteNotice(`Loaded page ${payload.page || 1} of ${payload.total_pages || 1}: ${payload.count || 0} of ${payload.total || 0} matching spans from ${payload.database_count || 0} SQLite database${payload.database_count === 1 ? "" : "s"}.`, "success");
    if ((payload.spans || []).length) await loadLiteTraceDetail(payload.spans[0], { requestToken });
  } catch (error) {
    if (requestToken !== liteTraceRequestToken) return;
    liteTraceListState = null;
    document.querySelector("#lite-database-badge").textContent = "UNAVAILABLE";
    document.querySelector("#lite-trace-count").textContent = "Span data unavailable";
    list.innerHTML = `<div class="lite-empty-state"><div class="empty-icon small">!</div><strong>Unable to load Lite spans</strong><span>${escapeHtml(error.message)}</span></div>`;
    setLiteNotice(error.message, "error");
  }
};

const loadLiteTraceDetail = async (span, { requestToken = null } = {}) => {
  if (!span?.trace_id || !span?.source) return;
  const detailToken = requestToken ?? ++liteTraceRequestToken;
  if (requestToken !== null) liteTraceRequestToken = requestToken;
  activeLiteTraceKey = liteTraceKey(span);
  activeLiteSpanId = null;
  ...
  try {
    const directory = document.querySelector("#lite-trace-directory").value.trim();
    const params = new URLSearchParams({ directory, source: span.source });
    if (span.span_id) params.set("span_id", span.span_id);
    const payload = await apiRequest(`/api/v1/lite/traces/${encodeURIComponent(span.trace_id)}?${params.toString()}`);
    if (detailToken !== liteTraceRequestToken) return;
    liteTraceDetailState = payload;
    activeLiteSpanId = payload.selected_span_id || span.span_id || payload.trace.root_span_id || payload.spans?.[0]?.span_id || null;
    liteExpandedSpanIds = defaultLiteExpandedSpanIds(payload, activeLiteSpanId);
...
const sortLiteSpans = (spans) => [...spans].sort((left, right) => {
...
const buildLiteSpanTree = (spans, rootSpanId) => {
  const spansById = new Map(spans.map((span) => [String(span.span_id), span]));
  const childrenByParent = new Map();
  spans.forEach((span) => {
    const parentId = span.parent_span_id == null ? null : String(span.parent_span_id);
    if (!parentId || !spansById.has(parentId) || parentId === String(span.span_id)) return;
    const children = childrenByParent.get(parentId) || [];
    children.push(span);
    childrenByParent.set(parentId, children);
  });
  childrenByParent.forEach((children, parentId) => {
    childrenByParent.set(parentId, sortLiteSpans(children));
  });

  const normalizedRootId = rootSpanId == null ? null : String(rootSpanId);
  const naturalRoots = spans.filter((span) => {
    const spanId = String(span.span_id);
    const parentId = span.parent_span_id == null ? null : String(span.parent_span_id);
    return spanId === normalizedRootId || !parentId || !spansById.has(parentId) || parentId === spanId;
  });
  const roots = sortLiteSpans(naturalRoots);
  if (normalizedRootId) {
    roots.sort((left, right) => {
      if (String(left.span_id) === normalizedRootId) return -1;
      if (String(right.span_id) === normalizedRootId) return 1;
      return 0;
    });
  }

  const reachable = new Set();
  const markReachable = (span) => {
    const spanId = String(span.span_id);
    if (reachable.has(spanId)) return;
    reachable.add(spanId);
    (childrenByParent.get(spanId) || []).forEach(markReachable);
  };
  roots.forEach(markReachable);
  sortLiteSpans(spans).forEach((span) => {
    if (!reachable.has(String(span.span_id))) {
      roots.push(span);
      markReachable(span);
    }
  });
  ...
  const visibleRows = [];
  const addVisible = (span, treeDepth, path) => {
    const spanId = String(span.span_id);
    if (path.has(spanId)) return;
    const children = childrenByParent.get(spanId) || [];
    visibleRows.push({ span, treeDepth, childCount: children.length });
    if (!liteExpandedSpanIds.has(spanId)) return;
    const nextPath = new Set(path);
    nextPath.add(spanId);
    children.forEach((child) => addVisible(child, treeDepth + 1, nextPath));
  };
  roots.forEach((root) => addVisible(root, 0, new Set()));
  return { childrenByParent, visibleRows };
};

const defaultLiteExpandedSpanIds = (payload, selectedSpanId) => {
  const expanded = new Set();
  const rootSpanId = payload?.trace?.root_span_id;
  if (rootSpanId) expanded.add(String(rootSpanId));
  const spansById = new Map((payload?.spans || []).map((span) => [String(span.span_id), span]));
  const visited = new Set();
  let currentId = selectedSpanId == null ? null : String(selectedSpanId);
  while (currentId && spansById.has(currentId) && !visited.has(currentId)) {
    visited.add(currentId);
    const parentSpanId = spansById.get(currentId).parent_span_id;
    if (parentSpanId == null || !spansById.has(String(parentSpanId))) break;
    currentId = String(parentSpanId);
...
const liteSpanAncestry = (payload, selectedSpanId) => {
  const spansById = new Map((payload?.spans || []).map((span) => [String(span.span_id), span]));
  const rootSpanId = String(payload?.trace?.root_span_id || "");
  const ancestry = [];
  const visited = new Set();
  let currentSpanId = selectedSpanId ? String(selectedSpanId) : "";
  while (currentSpanId && spansById.has(currentSpanId) && !visited.has(currentSpanId)) {
    visited.add(currentSpanId);
    const span = spansById.get(currentSpanId);
    ancestry.push(span);
    if (currentSpanId === rootSpanId) break;
    currentSpanId = span.parent_span_id ? String(span.parent_span_id) : "";
  }
  ancestry.reverse();
  return {
    ancestry,
    connectedToRoot: Boolean(ancestry.length && String(ancestry[0].span_id) === rootSpanId),
  };
};

const selectLiteSpan = (spanId) => {
  if (!spanId) return;
  activeLiteSpanId = String(spanId);
  document.querySelectorAll("#lite-flame-chart [data-span-id]").forEach((candidate) => {
    candidate.classList.toggle("selected", candidate.dataset.spanId === activeLiteSpanId);
  });
  renderLiteRootContext(liteTraceDetailState);
  renderLiteSpanDetail(activeLiteSpanId);
};

const toggleLiteSpanRow = (spanId) => {
  if (!spanId || !liteTraceDetailState) return;
  const normalizedSpanId = String(spanId);
  const { childrenByParent } = buildLiteSpanTree(
    liteTraceDetailState.spans || [],
    liteTraceDetailState.trace?.root_span_id,
  );
  if ((childrenByParent.get(normalizedSpanId) || []).length) {
    if (liteExpandedSpanIds.has(normalizedSpanId)) {
      liteExpandedSpanIds.delete(normalizedSpanId);
    } else {
      liteExpandedSpanIds.add(normalizedSpanId);
    }
  }
  activeLiteSpanId = normalizedSpanId;
  renderLiteFlameChart(liteTraceDetailState);
  renderLiteSpanDetail(activeLiteSpanId);
};

const renderLiteRootContext = (payload) => {
  const context = document.querySelector("#lite-root-context");
  const spans = payload?.spans || [];
  const rootSpan = spans.find((span) => String(span.span_id) === String(payload?.trace?.root_span_id))
    || payload?.root_span;
  const selectedSpan = spans.find((span) => String(span.span_id) === String(activeLiteSpanId));
  const rootStatus = payload?.trace?.root_span_status || (rootSpan ? "resolved" : "pending");
  context.classList.remove("pending", "missing");
  if (!selectedSpan) {
  ...
  if (!rootSpan) {
    const isPending = rootStatus === "pending";
    const label = isPending ? "ROOT SPAN PENDING" : "ROOT SPAN UNAVAILABLE";
    const summary = isPending ? "Trace in progress" : "Partial trace";
    const relation = isPending
      ? `Selected: ${selectedSpan.name} · only completed spans are visible until the true root span ends`
      : `Selected: ${selectedSpan.name} · the stored root span is unavailable in this database`;
    context.classList.add(isPending ? "pending" : "missing");
    context.innerHTML = `<div>
        <span class="lite-root-label">${label}</span>
        <strong class="lite-root-state">${summary}</strong>
      </div>
      <div class="lite-root-relation">${escapeHtml(relation)}</div>`;
    context.classList.remove("hidden");
    return;
  }
  const isRoot = String(rootSpan.span_id) === String(selectedSpan.span_id);
  const { ancestry, connectedToRoot } = liteSpanAncestry(payload, selectedSpan.span_id);
  const ancestryNames = ancestry.map((span) => span.name).join(" → ");
  const relation = isRoot
    ? "The selected span is this trace’s root span."
    : connectedToRoot
      ? `Selected: ${selectedSpan.name} · ${ancestryNames}`
      : `Selected: ${selectedSpan.name} · ancestry is incomplete in this database`;
  context.innerHTML = `<div>
      <span class="lite-root-label">ROOT SPAN</span>
      <button class="lite-root-button" type="button" data-focus-root-span="${escapeHtml(rootSpan.span_id)}">${escapeHtml(rootSpan.name)} · ${escapeHtml(formatDuration(rootSpan.duration_ms))}</button>
    </div>
    <div class="lite-root-relation">${escapeHtml(relation)}</div>`;
  context.classList.remove("hidden");
  context.querySelector("[data-focus-root-span]").addEventListener("click", () => selectLiteSpan(rootSpan.span_id));
};
...
const renderLiteFlameChart = (payload) => {
  ...
  const rootSpan = spans.find((span) => String(span.span_id) === String(trace.root_span_id));
  const rootStatus = trace.root_span_status || (rootSpan ? "resolved" : "pending");
  const isResolved = rootStatus === "resolved" && rootSpan;
  const heading = isResolved ? rootSpan.name : rootStatus === "pending" ? "Trace in progress" : "Partial trace";
  const rangeLabel = isResolved ? "Root span" : "Observed window";
  const statusLabel = isResolved ? (rootSpan.status_code || "UNSET") : rootStatus === "pending" ? "IN PROGRESS" : "PARTIAL";
  renderLiteRootContext(payload);
  document.querySelector("#lite-trace-heading h2").textContent = heading;
  document.querySelector("#lite-trace-heading .card-description").textContent = `${rangeLabel} · traceId ${trace.trace_id} · ${payload.source} · ${trace.span_count || spans.length} ${isResolved ? "spans" : "completed spans"} · ${formatDuration(trace.duration_ms)}`;
  document.querySelector("#lite-trace-status").textContent = statusLabel;
  if (!spans.length) {
  ...
  const { visibleRows } = buildLiteSpanTree(spans, trace.root_span_id);
  const rows = visibleRows.map(({ span, treeDepth, childCount }) => {
    ...
    const spanId = String(span.span_id);
    const selected = spanId === String(activeLiteSpanId) ? " selected" : "";
    const expanded = childCount > 0 && liteExpandedSpanIds.has(spanId);
    const ariaExpanded = childCount > 0 ? ` aria-expanded="${expanded}"` : "";
    const caret = childCount > 0 ? (expanded ? "▾" : "▸") : "·";
    const childLabel = childCount > 0 ? `${childCount} sub-span${childCount === 1 ? "" : "s"}` : "Leaf span";
    const title = `${span.name} · ${formatDuration(span.duration_ms)} · ${childLabel} · ${span.span_id}`;
    return `<button class="lite-flame-row${selected}" type="button" role="treeitem" aria-level="${treeDepth + 1}"${ariaExpanded} data-span-id="${escapeHtml(spanId)}" title="${escapeHtml(title)}">
      <span class="lite-flame-row-label" style="padding-left:${10 + Math.min(treeDepth, 8) * 15}px">
        <span class="lite-flame-caret" aria-hidden="true">${caret}</span>
        <span class="lite-flame-row-copy">
          <strong>${escapeHtml(span.name)}</strong>
          <small>${escapeHtml(formatDuration(span.duration_ms))} · ${escapeHtml(childLabel)}</small>
        </span>
      </span>
      <span class="lite-flame-timeline">
        <span class="lite-flame-gridline" style="left:25%"></span>
        <span class="lite-flame-gridline" style="left:50%"></span>
        <span class="lite-flame-gridline" style="left:75%"></span>
        <span class="lite-flame-block ${flameBlockTone(span)}" style="left:${left.toFixed(4)}%;width:${width.toFixed(4)}%"></span>
      </span>
    </button>`;
  }).join("");
  chart.innerHTML = `<div class="lite-flame-axis">
      <span class="lite-flame-axis-label">SPAN · CLICK TO EXPAND</span>
      <span class="lite-flame-axis-time"><span>0 ms</span><span>${escapeHtml(formatDuration(totalMs / 2))}</span><span>${escapeHtml(formatDuration(totalMs))}</span></span>
    </div>
    <div class="lite-flame-canvas" role="tree" aria-label="Trace span hierarchy">${rows}</div>`;
  chart.querySelectorAll(".lite-flame-row[data-span-id]").forEach((row) => {
    row.addEventListener("click", () => toggleLiteSpanRow(row.dataset.spanId));
  });
};
...
const renderLiteSpanDetail = (spanId) => {
  ...
  detail.innerHTML = `<div class="lite-span-detail-heading">
    ...
    <div class="lite-span-metrics">
      <div class="lite-span-metric lite-span-metric-time"><span>Generated at</span><strong><time datetime="${escapeHtml(span.start_time)}" title="${escapeHtml(span.start_time)}">${escapeHtml(formatTraceTime(span.start_time))}</time></strong></div>
...
document.querySelector("#lite-load-traces").addEventListener("click", () => loadLiteTraces({ page: 1 }));
document.querySelector("#lite-refresh-traces").addEventListener("click", () => loadLiteTraces({ page: liteSpanPage }));
document.querySelector("#lite-apply-filter").addEventListener("click", () => loadLiteTraces({ page: 1 }));
document.querySelector("#lite-clear-filter").addEventListener("click", () => {
  document.querySelector("#lite-span-name-filter").value = "";
  loadLiteTraces({ page: 1 });
});
document.querySelector("#lite-previous-page").addEventListener("click", () => {
  if (liteSpanPage > 1) loadLiteTraces({ page: liteSpanPage - 1 });
});
document.querySelector("#lite-next-page").addEventListener("click", () => loadLiteTraces({ page: liteSpanPage + 1 }));
document.querySelector("#lite-trace-limit").addEventListener("change", () => loadLiteTraces({ page: 1 }));
...
document.querySelector("#lite-span-name-filter").addEventListener("keydown", (event) => {
  if (event.key === "Enter") {
    event.preventDefault();
    loadLiteTraces({ page: 1 });
  }
