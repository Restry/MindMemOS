...
from ..text import estimate_tokens, get_text_preprocessor
from .scored_candidate import ScoredSearchCandidate, merge_scored_candidates


class _TextPreprocessor(Protocol):
...
class MemoryRetentionSelector:
    ...
    def __init__(
        ...
        *,
        ...
        text_preprocessor: _TextPreprocessor | None = None,
    ...
    def select(
        ...
        *,
        ...
        # The mixed score decides membership, while the existing relevance order
        # remains the stable presentation order.
    ...
    def _query_overlap(self, query_terms: set[str], text: str) -> float:
        if not query_terms:
            return 0.0
        processed = self._text_preprocessor.preprocess_text(text, include_entities=False)
        memory_terms = {str(token).casefold() for token in processed.tokens if str(token).strip()}
